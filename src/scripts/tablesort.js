...
var fdTableSort = {
        ...
        initSort: function(noCallback) {
                ...
                for(var i = 0; i < len1; i++) {
                        ...
                        if(!identical) {
                                ...
                                if (tr.parentNode != hook) {
                                        console.error("Error during table sort. Make sure the table's id is used only once in the page.")
                                        // Note: we leave the failing lines below anyway so they will crash in order to also show the proper error in the console.
                                }
        ...
        dateFormat: function(dateIn, favourDMY) {
                var dateTest = [
                        ...
                        { regExp:/^(0?[1-9]|1[012])([- \/.])((\d\d)?\d\d)$/, m:1, y:3 },   // my
                        { regExp:/^(0?[1-9]|[12][0-9]|3[01])([- \/.])(0?[1-9]|1[012])([- \/.])((\d\d)?\d\d)([- \/.:])([01][0-9]|2[0-4])([- \/.:])([0-5][0-9])([- \/.:])([0-5][0-9])$/, d:1, m:3, y:5, H:8, M:10, S:12 }  // dmy HMS
                        ];
                var start;
                var cnt = 0;
                ...
                while(cnt < numFormats) {
                        ...
                        if(dateIn.match(dateTest[start].regExp)) {
                                ...
                                if(m != null && m.length == 1) m = "0" + String(m);
                                if(d != null && d.length == 1) d = "0" + String(d);
                                if(y != null && y.length != 4) y = (parseInt(y) < 50) ? "20" + String(y) : "19" + String(y);
                                if(H != null && H.length == 1) H = "0" + String(H);
                                if(M != null && M.length == 1) M = "0" + String(M);
                                if(S != null && S.length == 1) S = "0" + String(S);

                                var result = "";
                                if(y != null) result += String(y);
                                if(m != null) result += String(m);
                                if(d != null) result += String(d);
                                if(H != null) result += String(H);
                                if(M != null) result += String(M);
        ...
        sortText:function (a,b) {
                var aa = fdTableSort.normalize(a[fdTableSort.pos]);
                var bb = fdTableSort.normalize(b[fdTableSort.pos]);
                if(aa == bb) return 0;
                if(aa < bb)  return -1;
                return 1;
        },
        ...
        normalize: function(s) {
                s = s.replace(/[àáâãäåæ]/g, 'a');
                s = s.replace(/[èéêë]/g, 'e');
                s = s.replace(/[ìíîï]/g, 'i');
                s = s.replace(/[ðñòóôõöø]/g, 'o');
